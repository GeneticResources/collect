...
#pragma omp parallel for
  ...
  Bolt::Bolt(const SnpData &_snpData, const DataMatrix &_covarDataT, const double _maskIndivs[],
	     ...
	     bool covarUseMissingIndic, int _mBlockMultX) :
    snpData(_snpData), covarDataT(_covarDataT),
    covBasis(_covarDataT, _maskIndivs, covars, covMaxLevels, covarUseMissingIndic),
    mBlockMultX(_mBlockMultX) { // block size for X, X' mult in CG... TODO: optimize for speed
    init();
  }
  ...
  (vector <double> pheno, vector <double> logDeltas, const vector <double> &sigma2Ks,
    ...
    */
    ...
    vector <double> proStats(numCalibSnps), retroStats(numCalibSnps);
    for (int j = 0; j < numCalibSnps; j++) {
      uint64 m = mProSnps[j];
...
//double *sigma2Kbest, double *logDeltaBest, double *bestAbsF,
  ...
  double Bolt::estLogDelta(double *sigma2Kbest, double HinvPhiCovCompVec[],
			   ...
			   double logDeltaTol, int CGmaxIters, double CGtol, int seed) const {
    ...
    */
    ...
    if (numChromsProjMask == 22)
      printf("Approximate standard error: 316/N = %.3f\n", 316.0 / (Nused-Cindep));
    cout << "To more precisely estimate variance parameters and s.e., use --reml" << endl;
    ...
    if (fabs(bestVCs.logDelta) == MAX_ABS_LOG_DELTA) {
      cerr << "ERROR: Invalid heritability estimate; cannot continue analysis" << endl;
      exit(1);
    }
  ...
  string Bolt::getSnpStats(const string &ID, int chrom,
	...
	//if (dotProd<0) stat = -stat;
      ...
      char pValueBuf[100]; sprintf(pValueBuf, "%.1E", pValue);
...
#pragma omp parallel for schedule(dynamic)
	...
	for (int b = 0; b < B; b++) {
	  ...
	  if (D > bufs[t].size()) bufs[t].resize(D);
	...
	B = 0; // reset current block size
      ...
      if (mbgen % 10000 == 9999)
	cout << "At SNP " << mbgen+1 << "; time: " << timer.update_time() << endl;
    }
