...
int main(int argc, char *argv[]) {
  ...
  cout << "                      +-----------------------------+" << endl;
  cout << "                      |                       ___   |" << endl;
  cout << "                      |   BOLT-LMM, v2.3_beta/_ /   |" << endl;
  cout << "                      |   July 26, 2017       /_/   |" << endl;
  cout << "                      |   Po-Ru Loh            //   |" << endl;
  cout << "                      |                        /    |" << endl;
  cout << "                      +-----------------------------+" << endl;
  cout << endl;
  /*
#ifdef USE_MKL
  cout << "Copyright (C) 2014-2017 Harvard University.  All rights reserved." << endl;
  cout << "Non-commercial use only." << endl << endl;
#else
  */
  cout << "Copyright (C) 2014-2017 Harvard University." << endl;
  ...
  SnpData snpData(params.famFile, params.bimFiles, params.bedFiles, params.geneticMapFile,
		  ...
		  params.remlGuessVCnames, !params.reml && params.lmmInf, params.allowX);
  ...
  if (params.lmmInf) {
	...
	VBiters = boltCV.estMixtureParams
	  (&f2Est, &pEst, &predBoost, pheno, logDeltaEst, sigma2Kest, params.CVfoldsSplit,
	   ...
	   maxItersCV, params.approxLLtol, params.mBlockMultX); // TODO: change MCMC param back to VB?
  ...
  if (!params.statsFileBgenSnps.empty()) {
	...
	bolt.streamBgen(params.statsFileBgenSnps, f, params.bgenFiles[f], params.sampleFile,
			params.bgenMinMAF, params.bgenMinINFO, params.geneticMapFile,
			params.verboseStats, retroData, params.domRecHetTest);
      else
	bolt.streamBgen2(params.statsFileBgenSnps, f, params.bgenFiles[f], params.sampleFile,
			 params.bgenMinMAF, params.bgenMinINFO, params.geneticMapFile,
