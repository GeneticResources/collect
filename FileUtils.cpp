...
namespace FileUtils {
  ...
  int checkBgenSample(const std::string &bgenFile, const std::string &sampleFile) {
    ...
    char snpID[65536], rsID[65536];
    ushort LS; fread(&LS, 2, 1, fin); // cout << "LS: " << LS << endl;
    fread(snpID, 1, LS, fin); snpID[LS] = '\0'; cout << "first snpID: " << string(snpID) << endl;
    //fseek(fin, LS, SEEK_CUR); // skip SNP id
    ushort LR; fread(&LR, 2, 1, fin); // cout << "LR: " << LR << endl;
