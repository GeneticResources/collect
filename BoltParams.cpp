/*
   ...
   developed by Po-Ru Loh.  Copyright (C) 2014-2015 Harvard University.
...
namespace LMM {
  ...
  bool BoltParams::processCommandLineArgs(int argc, char *argv[]) {
    ...
    typical.add_options()
      ...
      ("sampleFile", po::value<string>(&sampleFile),
       "file containing Oxford sample file corresponding to BGEN file")
    ...
    additional.add_options()
      ...
      ("allowX", "enable chrX analysis (recommended only for female-only traits)")
    ...
    hidden.add_options()
      ...
      ("highH2ChromMax", po::value<int>(&highH2ChromMax)->default_value(22),
	...
	cerr << "ERROR: --dosageFile(s), --dosageFidIidFile, and --statsFileDosageSnps must either"
	     << "       be all specified or all not specified" << endl;
	return false;
      }
      if (numDosageParams) {
	...
	if (isectIDs.size() < 0.5 * dosageIDs.size()) {
	  if (noDosageIDcheck)
	...
	for (uint64 i = 0; i < dosageFiles.size(); i++) {
	  ...
	  string chromStr; if (!(iss >> chromStr) || SnpData::plinkChromCode(chromStr) == -1) {
	...
	if (isectIDs.size() < 0.5 * impute2IDs.size()) {
	  if (noImpute2IDcheck)
	...
	while (finList >> chromStr >> file) {
	  ...
	  int chrom = SnpData::plinkChromCode(chromStr);
	  if (chrom == -1) {
	...
	for (uint64 i = 0; i < impute2Files.size(); i++) {
      ...
      int numBgenParams =
	!bgenFileTemplates.empty() + !sampleFile.empty() + !statsFileBgenSnps.empty();
      if (numBgenParams != 0 && numBgenParams != 3) {
	cerr << "ERROR: --bgenFile(s), --sampleFile, and --statsFileBgenSnps" << endl
	     << "        must either be all specified or all unspecified"
	     << endl;
	return false;
      }
	...
	if (isectIDs.size() < plinkIDs.size()) {
	  cerr << "ERROR: Some samples in --famFile/bfile are missing in --sampleFile"
	       << endl;
	  writeMissingIndivs(plinkIDs, sampleIDs);
	  return false;
	}
	if (isectIDs.size() < 0.5 * sampleIDs.size()) {
	  if (noBgenIDcheck)
	    cerr << "WARNING: Overlap between --sampleFile and --famFile is < 50%" << endl;
	  else {
	    cerr << "ERROR: Overlap between --sampleFile and --famFile is < 50%" << endl;
	...
	for (uint64 f = 0; f < bgenFiles.size(); f++) {
	  ...
	  bgenLayouts[f] = FileUtils::checkBgenSample(bgenFiles[f], sampleFile);
	}
      }
	...
	while (finList >> mapFile >> genoFile) {
	  ...
	  if (isectIDs.size() < 0.5 * dosage2IDs.size()) {
	    if (noDosage2IDcheck)
