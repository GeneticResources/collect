...
namespace LMM {
  ...
  int SnpData::plinkChromCode(const string &chrom) {
  ...
  vector <SnpInfo> SnpData::processSnps(vector <uint64> &Mfiles, const vector <string> &bimFiles,
    ...
    if (!allowX) {
      uint64 numAutosomalSnps = 0;
      for (uint64 mbed = 0; mbed < Mbed; mbed++) {
	int isAutosomal = (1 <= bedSnps[mbed].chrom && bedSnps[mbed].chrom <= 22);
	if (!isAutosomal) snpVCnum[mbed] = excludeVCnum;
	numAutosomalSnps += isAutosomal;
      }
      if (numAutosomalSnps < Mbed)
	cerr << "WARNING: Ignoring " << (Mbed - numAutosomalSnps) << " non-autosomal SNPs" << endl;
    }
  ...
  void SnpData::processMap(vector <SnpInfo> &bedSnps, const string &geneticMapFile,
    ...
    for (int mbedLast = (int) Mbed-1; mbedLast >= 0; mbedLast--)
      if (bedSnpToGrmIndex[mbedLast] != -2 && bedSnps[mbedLast].chrom <= 23) {
  ...
  SnpData::SnpData(const string &famFile, const vector <string> &bimFiles,
		   ...
		   vector <string> vcNamesIn, bool loadNonModelSnps, bool allowX) {
    ...
    vector <SnpInfo> bedSnps = processSnps(Mfiles, bimFiles, excludeFiles, modelSnpsFiles,
					   vcNamesIn, loadNonModelSnps, allowX);
  ...
  vector <SnpInfo> SnpData::readBimFile(const string &bimFile) {
    vector <SnpInfo> ret;
    string line;
